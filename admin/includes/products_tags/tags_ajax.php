<?php
	...
	if(isset($_POST['add'])){
		...
		$conn = $pdo->open();	
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM products_tags WHERE name=:name");
		...
		if($row['numrows'] > 0){			
			$msg="<div class='alert alert-danger alert-dismissible'>
				...
				<h4><i class='icon fa fa-warning'></i> Error!</h4>Tag already exist</div>";
				echo json_encode(array('sts'=>0,'msg'=>$msg));
		}else{		
			try{
				$stmt = $conn->prepare("INSERT INTO products_tags (name,createDate) 
		...
		$conn = $pdo->open();
		try{
			$stmt = $conn->prepare("DELETE FROM products_tags WHERE id=:id");
			$stmt->execute(['id'=>$id]);
			$msg="<div class='alert alert-success alert-dismissible'>
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
	...
	elseif(isset($_POST['edit'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM products_tags 
		...
		if($row['numrows'] > 0){			
			...
			try{
				$stmt = $conn->prepare("UPDATE products_tags SET name=:name,status=:status WHERE id=:id");
